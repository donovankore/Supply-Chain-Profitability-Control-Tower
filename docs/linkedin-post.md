...
I just completed a new end-to-end Business Analytics project: a **Supply Chain Profitability Control Tower**.

Using 35,958 validated order lines, I connected supplier performance, stockouts, forecasting, and on-time delivery to contribution margin—and identified a **$109K annualized recovery opportunity**.
...
1. Upload the social card from `public/og.png` as the first image if LinkedIn does not generate the link preview.
2. Add one dashboard screenshot as the second image.
3. Put the live demo link in the post and the GitHub link in the first comment if reach is a concern.
4. Tag the tools only if the target job description emphasizes them.
5. Reply to early comments with one concrete insight from the supplier scorecard.
